...
| `ghcr.io/valqore/engine-ai:1.12.1` | **Licensed** ([request access](mailto:tunc@valqore.io)) | Everything above + embedded offline AI model (AI scan, chat) |
...
docker run --rm -v valqore-data:/app/data ghcr.io/valqore/engine-ai:1.12.1 valqore activate YOUR_LICENSE_KEY
```
...
**AI image** (`ghcr.io/valqore/engine-ai:1.12.1`) — signed with Valqore's release key ([`cosign.pub`](cosign.pub)):

```bash
cosign verify --key cosign.pub --insecure-ignore-tlog ghcr.io/valqore/engine-ai:1.12.1
```
...
The `ghcr.io/valqore/engine-ai:1.12.1` image bundles a fine-tuned model that runs **fully offline** — your code never leaves the container. [Request a license.](mailto:tunc@valqore.io)

**AI scan — evaluate + drift + plain-English explanation in one shot:**

```bash
docker run --rm -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.12.1 valqore ai-scan ./ --state terraform.tfstate --cloud aws
```
...
docker run --rm -it -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.12.1 valqore chat deploy.yaml
```
...
docker run --rm -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.12.1 valqore ai-scan examples/ai-scan/
```
...
docker run --rm -it -v "$PWD:/work" -w /work -v valqore-data:/app/data \
  ghcr.io/valqore/engine-ai:1.12.1 valqore chat examples/chat/healthcare-api.yaml
