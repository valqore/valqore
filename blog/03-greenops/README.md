...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.12.0 valqore evaluate /workspace/high-carbon-deployment.yaml --score
```
...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.12.0 valqore evaluate /workspace/low-carbon-deployment.yaml --score
```
...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.12.0 valqore what-if /workspace/high-carbon-deployment.yaml --graviton
```
...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.12.0 valqore what-if /workspace/high-carbon-deployment.yaml --spot-ratio 70
```
