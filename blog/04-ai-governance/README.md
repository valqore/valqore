...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.11.0 valqore ai-gate /workspace/azure-llm-inference-ungoverned.yaml
```
...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.11.0 valqore ai-gate /workspace/azure-llm-inference-governed.yaml
```
...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.11.0 valqore evaluate /workspace/azure-llm-inference-ungoverned.yaml --score
```
...
docker run --rm \
  -v $(pwd):/workspace \
  ghcr.io/valqore/engine:1.11.0 valqore evidence eu_ai_act -f /workspace/
```
